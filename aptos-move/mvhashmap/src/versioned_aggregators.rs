...
enum AggregatorEntry {
    ...
    // transaction is estimated to populate the entry. In Block-STM, this happens when
    // to entries in the write-set when a speculative execution is aborted.
    // We also optionally keep a DeltaOp (and AggregatorID for the snapshot case) in the
    // Estimate, which allows a read to "bypass" waiting for the corresponding dependency,
    // by processing the stored bypass value instead.
    Estimate(Option<(DeltaOp, Option<AggregatorID>)>),
}
...
impl VersionedValue {
    // VersionedValue is only be created when base value of the corresponding aggregator
    // is known & provided to the constructor.
    fn new(base_value: Option<u128>) -> Self {
        Self {
            versioned_map: BTreeMap::new(),
            base_value,
            // Enable the optimization to not on dependencies during reading by default.
    ...
    fn mark_estimate(&mut self, txn_idx: TxnIndex) {
        ...
        match self.versioned_map.entry(txn_idx) {
            Entry::Occupied(mut o) => {
                let bypass = self
                    .read_estimate_deltas
                    .then_some(match &**o.get() {
                        Value(_, maybe_delta) => maybe_delta.map(|delta| (delta, None)),
                        Delta(delta) => Some((*delta, None)),
                        Snapshot(id, delta) => Some((*delta, Some(*id))),
        ...
        match self.versioned_map.entry(txn_idx) {
            Entry::Occupied(mut o) => {
                if !match (&**o.get(), &entry) {
                    ...
                    (Estimate(Some((delta_l, Some(id_l)))), Snapshot(id_r, delta_r)) => {
                        *id_l == *id_r && *delta_l == *delta_r
                    },
                    (Estimate(Some((delta_l, None))), Delta(delta_r) | Value(_, Some(delta_r))) => {
                        *delta_l == *delta_r
                    },
                    (Estimate(Some((_, None))), Snapshot(_, _)) => unreachable!(
			 ...
			 that previously contained a delta"
                    ...
                    (Estimate(None), Value(_, None)) => true,
			 ...
			 or we should be recording committed value"
                    ),
                } {
                    self.read_estimate_deltas = false;
                }
    ...
    fn apply_delta_suffix(
        ...
        let mut accumulator: Result<DeltaOp, ()> = Ok(delta);
        while let Some((idx, entry)) = iter.next_back() {
            let delta = match &**entry {
                ...
                Estimate(bypass) => {
                    if !self.read_estimate_deltas || bypass.is_none() {
                        // We must wait on Estimates, or a bypass isn't available.
                        return Err(MVAggregatorsError::Dependency(*idx));
                    }
                    let (bypass_delta, maybe_bypass_id) = bypass.expect("None already handled");
                    assert_none!(
                        maybe_bypass_id,
                        "Bypass previously stored for Aggregator ID that contains deltas"
                    );
                    bypass_delta
                },
            };
            ...
            |(idx, entry)| match &**entry {
                ...
                Estimate(bypass) => {
                    if !self.read_estimate_deltas || bypass.is_none() {
                        ...
                        let (delta, maybe_id) = bypass.expect("None handled in previous branch");
                        match maybe_id {
                            Some(id) => Ok(VersionedRead::Snapshot(id, *idx, delta)),
                            None => self
                                .apply_delta_suffix(&mut iter, delta)
                                .map(VersionedRead::Value),
                        }
...
impl VersionedAggregators {
    ...
    /// base value. This ensures that VersionedValue exists for thez aggregator before any
    ...
    pub fn read_committed_value(&self, id: AggregatorID) -> Result<u128, MVAggregatorsError> {
        self.values
            .get_mut(&id)
            .expect("VersionedValue for an (resolved) ID must already exist")
            .read_committed_value(self.next_idx_to_commit.load(Ordering::Relaxed))
    }
    ...
    /// read_committed_value, called frequently (as a part of aggregator implementation),
...
mod test {
    ...
    fn aggregator_entry(type_index: usize) -> Option<AggregatorEntry> {
        match type_index {
            ...
            ESTIMATE => Some(AggregatorEntry::Estimate(None)),
    ...
    fn estimate_bypass() {
        ...
        assert_matches!(
            &**val_bypass.unwrap(),
            AggregatorEntry::Estimate(Some((_, None)))
        );
        ...
        assert_matches!(&**val_no_bypass.unwrap(), AggregatorEntry::Estimate(None));
        ...
        assert_matches!(
            ...
            AggregatorEntry::Estimate(Some((_, Some(_))))
        );
    ...
    fn read_committed_not_value(type_index: usize) {
        ...
        let _ = v.read_committed_value(11);
    }
    ...
    fn read_committed_value() {
        ...
        assert_ok_eq!(v.read_committed_value(5), 15);
        assert_ok_eq!(v.read_committed_value(4), 10);
        assert_ok_eq!(v.read_committed_value(2), 5);
    }
